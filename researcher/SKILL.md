---
name: researcher
description: 构建并维护持久化个人知识库，专为深度研究某一技术领域的架构、模型、原理、机制和落地实践而设计。内部使用 LLM Wiki 模式组织知识，支持持续学习和知识复合积累。触发场景：(1) 'researcher:init' 初始化研究仓库；(2) 'researcher:research [文件/网址/代码库]' 摄入源并自动组织知识；(3) 'researcher:consult [查询]' 从知识库中综合回答，输出完整专业文档。适用于：研究技术领域、分析代码库、快速探索陌生领域、沉淀日常技术灵感、演进 idea。
---
...
## 阶段 1：初始化 (`researcher:init`)

1. 询问仓库路径（默认 `./research`）
2. 运行 `scripts/init_repo.py <path>`
3. 确认结构并说明后续步骤

---

## 阶段 2：摄入源 (`researcher:research <source>`)
...
## 阶段 3：查询知识 (`researcher:consult <query>`)
